...
func (h *JobResearchHandler) GetJobResearch(c *gin.Context) {
    db := c.MustGet("db").(*mongo.Database)
    researchColl := db.Collection("job_research_results")
    ...
    if userID == "" || jobID == "" {
        c.JSON(http.StatusBadRequest, gin.H{"error": "Missing user or job ID"})
        return
    }

    var result models.JobResearchResult
    err := researchColl.FindOne(c, bson.M{
        "auth_user_id": userID,
        "job_id":       jobID,
    }).Decode(&result)
    ...
    if err != nil {
        if err == mongo.ErrNoDocuments {
            c.JSON(http.StatusNotFound, gin.H{"error": "No job research result found for this user and job"})
        } else {
            c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to fetch job research result"})
        }
        return
    }
